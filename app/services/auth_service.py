...
async def register_service(session: AsyncSession, user_create: UserCreate) -> User:
    """
    Handle public user registration.
    Orchestrates user creation and any post-registration tasks.
    """
    return await create_user_service(session, user_create)
...
async def login_service(session: AsyncSession, email: str, password: str) -> Token:
    """
